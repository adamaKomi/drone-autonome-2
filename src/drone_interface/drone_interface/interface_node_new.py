...
class DroneInterface(Node):
    ...
    def _setup_services(self):
        ...
        self.control_service = self.create_service(
            String,  # Utilise String pour simplicité, remplacer par DroneControl plus tard
            '/drone/control',
        ...
        self.safety_service = self.create_service(
            String,  # Utilise String pour simplicité, remplacer par SafetyCheck plus tard
            '/drone/safety_check',
    ...
    def _handle_control_request(self, request, response):
        """Gère les requêtes de contrôle du drone"""
        try:
            # Parse de la commande (format simple pour l'instant)
            command_parts = request.data.split(':')
            command_type = command_parts[0].upper()
            
            self.logger.info(f"🎮 Commande reçue: {command_type}")
            ...
            if command_type == 'ARM':
                ...
                if len(command_parts) > 1:
                    ...
                    success, message = self._handle_mode_command(mode)
                else:
                    success, message = False, "Mode parameter required"
            elif command_type == 'TAKEOFF':
                altitude = float(command_parts[1]) if len(command_parts) > 1 else 2.5
                success, message = self._handle_takeoff_command(altitude)
            elif command_type == 'LAND':
                success, message = self._handle_land_command()
            elif command_type == 'EMERGENCY':
                success, message = self._handle_emergency_command()
            else:
                success, message = False, f"Unknown command: {command_type}"
                
            # Préparer la réponse
            result = "SUCCESS" if success else "FAILED"
            response.data = f"{result}:{message}"
            
        except Exception as e:
            self.logger.error(f"❌ Erreur dans handle_control_request: {e}")
            response.data = f"FAILED:Internal error: {str(e)}"
    ...
    def _handle_safety_request(self, request, response):
        ...
        try:
            ...
            safety_info = {
                ...
                'arm_reason': reason,
                'flight_safe': is_safe,
                'warnings': warnings + flight_warnings,
                'errors': flight_errors
            }
            
            response.data = str(safety_info)  # Simplification, à améliorer avec des messages structurés
    ...
    def _print_node_info(self):
        ...
        self.logger.info("  • /drone/control - Contrôle du drone")
        ...
        self.logger.info("  ros2 service call /drone/control std_msgs/srv/SetString '{data: \"ARM\"}'")
        self.logger.info("  ros2 service call /drone/control std_msgs/srv/SetString '{data: \"SET_MODE:GUIDED\"}'")
        self.logger.info("  ros2 service call /drone/control std_msgs/srv/SetString '{data: \"TAKEOFF:3.0\"}'")
        self.logger.info("=" * 60)
