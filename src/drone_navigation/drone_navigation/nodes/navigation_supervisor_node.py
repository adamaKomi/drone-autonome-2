...
class NavigationSupervisorNode(Node):
    def __init__(self):
        ...
        self.create_subscription(
            NavigationStatus, '/drone_nav/gps_status', self._nav_status_callback, 10
        )
        ...
        self.create_subscription(
            EmergencyStatus, '/drone_nav/emergency_status', self._emergency_status_callback, 10
        )
    ...
    def _trigger_emergency_stop(self, reason="Unknown"):
        ...
        self.get_logger().critical(f"ARRÊT D'URGENCE DÉCLENCHÉ: {reason}")
